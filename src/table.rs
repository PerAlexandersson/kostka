...
fn run_full_matrix(n: u32, args: &TableArgs) {
    ...
    for lam in &shapes {
        ...
        let row: Vec<_> = weights
            .par_iter()
            .map(|w| kostka(lam, w.parts(), max_states))
            .collect();
...
fn run_all_weights(args: TableArgs) {
    let lambda_str = args.lambda.as_deref().unwrap_or_else(|| {
        eprintln!("--lambda required");
        std::process::exit(1);
    });
    let lam = parse_partition(lambda_str).unwrap_or_else(|e| {
        eprintln!("{}", e);
        std::process::exit(1);
    });
    let inner = if args.mu.is_empty() {
        Partition::empty()
    } else {
        parse_partition(&args.mu).unwrap_or_else(|e| {
            eprintln!("{}", e);
            std::process::exit(1);
        })
    };
    ...
    if args.format == "text" {
        if args.ehrhart {
            println!(
                "Ehrhart polynomials for GT( {} | w )  (|w| = {}):\n",
                shape_str, skew_size
            );
        } else {
            println!(
                "K( {} | w ) for all weights w of size {}:\n",
                shape_str, skew_size
            );
        }
    }
    ...
    let max_states = args.max_states;
    let mut rows: Vec<TableRow> = weights
        .par_iter()
        .map(|w| {
            let k = skew_kostka(&lam, &inner, w, max_states, true);
            let (poly, hv) = if args.ehrhart
                || args.hstar
                || args.min_degree.is_some()
                || args.max_degree.is_some()
                || args.palindromic
                || args.unimodal
            {
                let p = compute_ehrhart(&lam, &inner, w, None, None, false, max_states, true);
                let h = if args.hstar || args.palindromic || args.unimodal {
                    Some(compute_hstar(&p))
                } else {
                    None
                };
                (Some(p), h)
    ...
    rows.retain(|row| {
        if let Some(p) = &row.poly {
            if let Some(min_d) = args.min_degree {
                if p.degree < min_d {
                    return false;
                }
            }
            if let Some(max_d) = args.max_degree {
                if p.degree > max_d {
                    return false;
                }
            }
        ...
        if let Some(h) = &row.hstar {
            if args.palindromic && !is_palindromic(h) {
                return false;
            }
            if args.unimodal && !is_unimodal(h) {
                return false;
            }
        }
        true
    ...
    match args.format.as_str() {
        ...
        "json" => {
            ...
            let hstar_str = row
                ...
                .unwrap_or("null".into());
            println!(
                "{{\"weight\":[{}],\"value\":\"{}\",\"degree\":{},\"polynomial\":{},\"hstar\":{}}}",
                w_str, row.kostka_val, deg_str, poly_str, hstar_str
            );
        }
        _ => {
