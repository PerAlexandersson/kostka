...
% =====================================================================
\section{The \texorpdfstring{$h^{*}$}{h-star}-vector}\label{sec:hstar}
% =====================================================================
...
\subsection{Non-real-rooted $h^*$-vectors from permutation posets}\label{subs:nonRR}
...
(log-concave but \emph{not} real-rooted),
we searched over 34{,}226 nearby $4321$-avoiding permutations
(within 3~arbitrary transpositions) and found exactly 22 with
non-real-rooted~$h^*$.
Deletion and standardization (removing a value and relabeling)
was used to search downward; no non-real-rooted examples exist
at $n = 16$ among $21{,}253$ candidates tested.
Thus $n = 17$ appears to be a sharp threshold for this family.

Iterating insertions of $n+1$ at various positions produces
non-real-rooted examples at each $n \geq 17$, with the imaginary
part of the unique conjugate pair of non-real roots growing
approximately linearly with~$n$.
All non-real-rooted examples have exactly two non-real roots.
...
\[
  w = (3,4,6,8,10,12,2,1,15,5,17,7,9,11,13,14,16) \in S_{17},
\]
which \emph{contains} the pattern~$321$ (at positions $6,7,8$:
the subsequence $12, 2, 1$).  Its $h^*$-polynomial is
\[
...
\begin{question}\label{q:minimalNonRR}
Is $n = 17$ the smallest value of~$n$ for which a permutation poset
$P_w$ has a non-real-rooted $h^*$-polynomial?
\end{question}
