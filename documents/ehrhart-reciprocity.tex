...
and $h^*$-vector computation to Normaliz~\cite{BradenMacPhersonProceesi2006};
...
% =====================================================================
\section{The \texorpdfstring{h-star}{$h^{*}$}-vector}\label{sec:hstar}
% =====================================================================
...
We benchmark our frontier DP implementation against polymake/Normaliz
for $h^*$-vector computation.
...
\begin{remark}\label{rem:linextVsReciprocity}
One might consider computing the $h^*$-vector directly from Stanley's
formula by enumerating all linear extensions and tabulating descents.
However, counting linear extensions is $\#\mathsf{P}$-complete in
...
$O(n \cdot k^w)$.  For narrow posets this is polynomial in~$n$, whereas
the linear extension approach is necessarily exponential in $|\mathcal{L}(P)|$.
\end{remark}
