...
yields speedups of $10^3$--$10^4\times$ compared to general-purpose
polytope software.  We benchmark against Oscar/polymake
 and identify other families of polytopes amenable to the same approach.
\end{abstract}
...
negative side of the polynomial for free or cheaply.
...
Recall that a \defin{semistandard Young tableau} (SSYT) of skew shape
...
precisely the \defin{strict GT patterns}: those where all interlacing
inequalities are strict.  Thus
...
(equivalently, GT patterns with strict inequalities at all internal
entries).  This is computed by the same DP as the ordinary Kostka
...
which uses Normaliz for general lattice-point enumeration, and
A.~Buch's \texttt{lrcalc}~\cite{Buch2000}, which reduces Kostka
...
\caption{%
  ...
  different hardware makes raw speedup an upper bound).
...
The speedup is consistently $10^4$--$10^5 \times$.
...
% =====================================================================
\section{The $h^{*}$-vector}\label{sec:hstar}
% =====================================================================
...
Our computations verify both properties for all GT polytopes
with $|\lam| \leq 15$ and standard weight---over 54{,}000
cases---providing further evidence for the
conjecture. 
...
\begin{example}\label{ex:hstarVectors}
For standard weight $\wt = (1^{|\lam|})$:
\begin{align*}
  \lam = (3,2,1),\; d=7: \quad & \hvec = (1, 8, 35, 32, 9) \\
  \lam = (3,3,3),\; d=10: \quad & \hvec = (1, 31, 469, 2113, 3466, 2113, 469, 31, 1) \\
  \lam = (4,3,2,1),\; d=21: \quad & \hvec = (1, 746, 214671, \dotsc, 2003616)
\end{align*}
The $h^*$-vector for the staircase $(4,3,2,1)$ has 16 nonzero entries.
\end{example}
...
shape $(\lambda_1, \dotsc, \lambda_r)$, $w \leq \max_i \lambda_i$.

\subsection{Order polytope benchmarks}\label{subs:orderBenchmarks}

We benchmark our frontier DP implementation (in the
\texttt{combinatoric-core} Rust crate) against polymake/Normaliz
for $h^*$-vector computation.  All $h^*$-vectors agree.
...
\caption{%
  ...
  ``Rust DP'' is our frontier DP with reciprocity and BigRational
  interpolation.  ``polymake'' uses the polymake/Normaliz backend.
  Both run on the same machine (desktop, single thread).
}
\label{tab:orderPolyBench}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lrrrr@{}}
\toprule
Poset & $n$ & Rust DP & polymake & speedup \\
\midrule
Chain(4)           &  4 & 0.2\,ms & 0.06\,s &    $4 \times 10^2$ \\
Chain(8)           &  8 & 0.9\,ms & 0.07\,s &    $80\times$       \\
Antichain(3)       &  3 & 0.08\,ms & 0.08\,s &   $10^3$           \\
Antichain(5)       &  5 & 0.2\,ms & 0.07\,s &    $4 \times 10^2$ \\
Shape $(3,2,1)$    &  6 & 0.4\,ms & 0.06\,s &    $2 \times 10^2$ \\
Shape $(4,3,2,1)$  & 10 & 1.9\,ms & 0.08\,s &    $40\times$       \\
Shape $(3,3,3)$    &  9 & 1.3\,ms & 0.08\,s &    $60\times$       \\
Fence(6)           &  6 & 0.7\,ms & 0.08\,s &    $10^2$           \\
Fence(8)           &  8 & 1.8\,ms & 0.20\,s &    $10^2$           \\
Fence(10)          & 10 & 2.1\,ms & 0.53\,s &    $3 \times 10^2$ \\
2-alternating(8)   &  8 & 0.9\,ms & 0.08\,s &    $90\times$       \\
3-alternating(9)   &  9 & 1.3\,ms & 0.08\,s &    $60\times$       \\
\bottomrule
\end{tabular*}
\end{table}

\begin{remark}\label{rem:orderPolySpeedup}
The speedup ranges from $40\times$ to $1000\times$ depending on
the poset structure.  Note that the polymake timings include a
roughly $50$\,ms startup overhead; for larger posets the gap widens
as Normaliz's triangulation-based approach scales with the number of
vertices, while the frontier DP scales with the frontier width.
For \emph{narrow} posets (chains, fences, zigzag structures),
where the frontier width $w$ is constant, the DP runs in polynomial time
$O(nk^{w})$, whereas the polytope has exponentially many vertices.
\end{remark}
...
$\mathrm{des}(\sigma)$ counts descents.  Our computation verifies this
identity for all posets in Table~\ref{tab:orderPolyBench}.
...
\]
This is palindromic, as expected for a Gorenstein polytope, and
was previously intractable with \texttt{i64} rational interpolation
due to overflow in the Lagrange interpolation step (now resolved by
using exact \texttt{BigRational} arithmetic).
\end{example}
